...
class QuerySet:
    ...
    def json_append(self, **kw):
        """
        功能:json数据追加
        """
        if not kw:
            return 0
        cond, cond_val = self.make_where(self.cond_list, self.cond_dict, self.exclude_list, self.exclude_dict)
        update_fields = u', '.join(u"{}=JSON_ARRAY_INSERT({}, '$[0]', CAST('{}' AS JSON))".format(k, k, v)
                                   for k, v in kw.items())
        sql = u"update {} set {} {}".format(self.table_name, update_fields, cond)
        n, _ = self.conn.execute(sql, *cond_val)
        return n
